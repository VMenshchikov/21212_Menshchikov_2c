...
class TFactory {
  public:
    ...
    bool Unregistred(KeyType Key) { return GetInstance()->FMap.erase(Key); }
    ...
    ReturnType GetObject(KeyType Key) {
        if (FMap.find(Key) == FMap.end())
            throw(std::invalid_argument("key not find")); //долго, оставить ли проверку?
        return FMap[Key]();
    }
  ...
  private:
    ...
    operator=(TFactory &b) = delete;
    TFactory<KeyType, ReturnType, ReturnObjectCreator>(){};
    TFactory<KeyType, ReturnType, ReturnObjectCreator>(
        TFactory<KeyType, ReturnType, ReturnObjectCreator> &b) = delete;
};
